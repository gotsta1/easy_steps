from app.main import (
    ...
    _should_send_post_kick_notification,
)
...
def test_settings_expose_only_selected_notification_thresholds() -> None:
    settings = Settings.model_construct(
        ...
        BOTHELP_STEP_NOTIFY_KICKED_10H="step-kicked-10h",
        BOTHELP_STEP_NOTIFY_KICKED_3D="step-kicked-3d",
    )

    assert settings.notify_steps_map == {3: "step-3d", 2: "step-2d"}
    assert settings.notify_post_kick_hours_map == {
        10: "step-kicked-10h",
        72: "step-kicked-3d",
    }
...
def test_only_selected_post_kick_notifications_are_enabled() -> None:
    for duration_days in (7, 30, 90, 180, 365, None):
        assert _should_send_post_kick_notification(duration_days, 10) is True
        assert _should_send_post_kick_notification(duration_days, 72) is True
        assert _should_send_post_kick_notification(duration_days, 168) is False
